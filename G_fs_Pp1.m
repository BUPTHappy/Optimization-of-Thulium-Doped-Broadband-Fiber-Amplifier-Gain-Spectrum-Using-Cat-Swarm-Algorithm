...
Pp_0=100e-3; % 980nm main pump power at input end: 200mW
Pase_0=0;     % ASE in signal bandwidth power at input end: 0
N=0.8*1e25;

%get the emission cross section sigma_se
%x=[1200:13.8889:1620];
%y=normpdf(x,1380,255);
...
G=zeros(1,len);
for count=1:len
    [z,P]=ode45(@fun,[0,L],[Ps_0,Pp_0,Pase_0],'AbsTol',N,swl(count),sig_se(count),sig_sa(count));
    G1(1,count)=10*log10(P(length(P),1)./Ps_0);
    G11(1,count)=P(length(P),1)./Ps_0;
    Ps(1,count)=P(length(P),1);
    Pase(1,count)=P(length(P),3);
    P3(1,count)=P(length(P),3);  
    f(count)=c/swl(count);
    delta_f=c*10^(-9)/swl(count)^2;
    NF1(1,count)=10*log10(1/G11(1,count)+P3(1,count)/(G11(1,count)*h*delta_f*f(count)));  
end
